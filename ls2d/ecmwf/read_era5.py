...
class Slice:
    ...
    def __call__(self, dj, di):
        return np.s_[:, :, self.jstart + dj : self.jend + dj, self.istart + di : self.iend + di]
...
class Read_era5:
    ...
    def open_netcdf_files(self):
        ...
        an_pres_files = [
            era_tools.era5_file_path(d.year, d.month, d.day, path, case, 'pressure_an', False) for d in an_dates
        ]
    ...
    def read_data(self):
        ...
        start_h_since = (self.start - date_00).total_seconds() / 3600.0
        end_h_since = (self.end - date_00).total_seconds() / 3600.0
        ...
        self.time_sec = (self.time - self.time[0]) * 3600.0
    ...
    def calc_derived_data(self):
        ...
        self.Tv = ifs_tools.calc_virtual_temp(
            self.T, self.q, self.qc, self.qi, self.qr, self.qs
        )  # Virtual temp on full levels (K)
        ...
        self.thl = (
            self.th - ifs_tools.Lv / (ifs_tools.cpd * self.exn) * self.ql
        ...
        self.U = (self.u**2.0 + self.v**2) ** 0.5  # Absolute horizontal wind (m s-1)
    ...
    def calculate_forcings(self, n_av=0, method='4th'):
        ...
        distance = spatial.haversine(
            self.lons[self.i],
            self.lats[self.j],
            self.settings['central_lon'],
            self.settings['central_lat'],
        )
        ...
        dx = spatial.dlon(self.lons[self.i - 1], self.lons[self.i + 1], self.lats[self.j]) / 2.0
        dy = spatial.dlat(self.lats[self.j - 1], self.lats[self.j + 1]) / 2.0
                ...
                for j in range(self.nlat):
                    for i in range(self.nlon):
                        self.ug[t, :, j, i] = interpolate.interp1d(
                            self.p_p, self.ug_p[t, :, j, i], fill_value='extrapolate'
                        )(self.p[t, :, j, i])
                        self.vg[t, :, j, i] = interpolate.interp1d(
                            self.p_p, self.vg_p[t, :, j, i], fill_value='extrapolate'
                        )(self.p[t, :, j, i])
        ...
        for t in range(self.ntime):
            self.ug_mean[t, :] = interpolate.interp1d(self.p_p, ug_p_mean[t, :], fill_value='extrapolate')(
                self.p_mean[t, :]
            )
            self.vg_mean[t, :] = interpolate.interp1d(self.p_p, vg_p_mean[t, :], fill_value='extrapolate')(
                self.p_mean[t, :]
            )
    ...
    def get_les_input(self, z):
        ...
        variables = {
            ...
            'p': ('air pressure', 'Pa'),
            'dtthl_advec': (
                'advective tendency liquid water potential temperature',
                'K s-1',
            ),
        ...
        add_ds_var(
            ...
            self.time_sec,
            ('time'),
            'seconds since start of experiment',
            's',
        )
        ...
        add_ds_var(
            ...
            self.T_mean,
            ('time', 'lay'),
            'full level temperature radiation',
            'K',
        )
        add_ds_var(
            ds,
            ...
            self.Th_mean,
            ('time', 'lev'),
            'half level temperature radiation',
            'K',
        )
        ...
        add_ds_var(
            ...
            self.o3_mean,
            ('time', 'lay'),
            'ozone volume mixing ratio radiation',
            'ppmv',
        )
        ...
        add_ds_var(
            ...
            self.soil_type_nn,
            None,
            'ECMWF soil type (Fortran indexing!)',
            '-',
        )
        ...
        add_ds_var(
            ds,
            'type_low_veg',
            self.veg_type_low_nn,
            None,
            'ECMWF low vegetation type (Fortran indexing!)',
            '-',
        )
        add_ds_var(
            ds,
            'type_high_veg',
            self.veg_type_high_nn,
            None,
            'ECMWF high vegetation type (Fortran indexing!)',
            '-',
        )
        ...
        add_ds_var(
            ds,
            'root_frac_low_veg',
            self.root_frac_low_nn,
            ('zs'),
            'root fraction low vegetation',
            '-',
        )
        add_ds_var(
            ds,
            'root_frac_high_veg',
            self.root_frac_high_nn,
            ('zs'),
            'root fraction high vegetation',
            '-',
        )
        ...
        ds.attrs['reference'] = (
            'van Stratum et al. (2023). The benefits and challenges of downscaling a global reanalysis with doubly-periodic large-eddy simulations. '
            'JAMES, https://doi.org/10.1029/2023MS003750'
        )
